...
//C# TO C++ CONVERTER TODO TASK: There is no built-in support for multithreading in native C++:
	...
	bool AllDraw::FoundOfCurrentTableNodeSolderIJ(int i, int j, int** Tab, int Order, AllDraw *THIS, bool &Found)
	{
		...
		if (SolderesOnTable[i]->SoldierThinking->Deep.empty())
		{
			...
			SolderesOnTable[i]->SoldierThinking->Deep[j]->SetRowColumn(0);
	...
	bool AllDraw::FoundOfCurrentTableNodeElephantIJ(int i, int j, int** Tab, int Order, AllDraw *THIS, bool &Found)
	{
		...
		if (ElephantOnTable[i]->ElefantThinking->Deep.empty())
		{
			...
			ElephantOnTable[i]->ElefantThinking->Deep[j]->SetRowColumn(0);
	...
	bool AllDraw::FoundOfCurrentTableNodeHourseIJ(int i, int j, int** Tab, int Order, AllDraw *THIS, bool &Found)
	{
		...
		if (HoursesOnTable[i]->HourseThinking->Deep.empty())
		{
			...
			HoursesOnTable[i]->HourseThinking->Deep[j]->SetRowColumn(0);
	...
	bool AllDraw::FoundOfCurrentTableNodeCastleIJ(int i, int j, int** Tab, int Order, AllDraw *THIS, bool &Found)
	{
		...
		if (CastlesOnTable[i]->CastleThinking->Deep.empty())
		{
			...
			CastlesOnTable[i]->CastleThinking->Deep[j]->SetRowColumn(0);
	...
	bool AllDraw::FoundOfCurrentTableNodeMinisterIJ(int i, int j, int** Tab, int Order, AllDraw *THIS, bool &Found)
	{
		...
		if (MinisterOnTable[i]->MinisterThinking->Deep.empty())
		{
			...
			MinisterOnTable[i]->MinisterThinking->Deep[j]->SetRowColumn(0);
	...
	bool AllDraw::FoundOfCurrentTableNodeKingIJ(int i, int j, int** Tab, int Order, AllDraw *THIS, bool &Found)
	{
		...
		if (KingOnTable[i]->KingThinking->Deep.empty())
		{
			...
			KingOnTable[i]->KingThinking->Deep[j]->SetRowColumn(0);
...
//ORIGINAL LINE: int[] Lose = MinisterOnTable[iIndex]->LoseOcuuredatChiled;
			...
			if (KindIndex == 1 || KindIndex == -1)
			{
				...
				SolderesOnTable[iIndex]->SoldierThinking->Deep[SolderesOnTable[iIndex]->SoldierThinking->Deep.size() - 1]->SetRowColumn(0);
			...
			else
			{
				...
				ElephantOnTable[iIndex]->ElefantThinking->Deep[ElephantOnTable[iIndex]->ElefantThinking->Deep.size() - 1]->SetRowColumn(0);
			...
			else
			{
				...
				HoursesOnTable[iIndex]->HourseThinking->Deep[HoursesOnTable[iIndex]->HourseThinking->Deep.size() - 1]->SetRowColumn(0);
			...
			else
			{
				...
				CastlesOnTable[iIndex]->CastleThinking->Deep[CastlesOnTable[iIndex]->CastleThinking->Deep.size() - 1]->SetRowColumn(0);
			...
			else
			{
				...
				MinisterOnTable[iIndex]->MinisterThinking->Deep[MinisterOnTable[iIndex]->MinisterThinking->Deep.size() - 1]->SetRowColumn(0);
			...
			else
			{
				...
				KingOnTable[iIndex]->KingThinking->Deep[MinisterOnTable[iIndex]->MinisterThinking->Deep.size() - 1]->SetRowColumn(0);
...
//C# TO C++ CONVERTER TODO TASK: There is no built-in support for multithreading in native C++:
		////lock (O1)
		{
			...
			if (Index[0]!= -1)
			{
				...
				SolderesOnTable[Index[0]]->SoldierThinking->Deep[SolderesOnTable[Index[0]]->SoldierThinking->Deep.size() - 1]->SetRowColumn(0);
...
//C# TO C++ CONVERTER TODO TASK: There is no built-in support for multithreading in native C++:
		////lock (O1)
		{
			...
			if (Index[1] != -1)
			{
				...
				ElephantOnTable[Index[1]]->ElefantThinking->Deep[ElephantOnTable[Index[1]]->ElefantThinking->Deep.size() - 1]->SetRowColumn(0);
...
//C# TO C++ CONVERTER TODO TASK: There is no built-in support for multithreading in native C++:
		////lock (O1)
		{
			...
			if (Index[2] != -1)
			{
				...
				HoursesOnTable[Index[2]]->HourseThinking->Deep[HoursesOnTable[Index[2]]->HourseThinking->Deep.size() - 1]->SetRowColumn(0);
...
//C# TO C++ CONVERTER TODO TASK: There is no built-in support for multithreading in native C++:
		////lock (O1)
		{
			...
			if (Index[3] != -1)
			{
				...
				CastlesOnTable[Index[3]]->CastleThinking->Deep[CastlesOnTable[Index[3]]->CastleThinking->Deep.size() - 1]->SetRowColumn(0);
...
//C# TO C++ CONVERTER TODO TASK: There is no built-in support for multithreading in native C++:
		////lock (O1)
		{
			...
			if (Index[4] != -1)
			{
				...
				MinisterOnTable[Index[4]]->MinisterThinking->Deep[MinisterOnTable[Index[4]]->MinisterThinking->Deep.size() - 1]->SetRowColumn(0);
...
//C# TO C++ CONVERTER TODO TASK: There is no built-in support for multithreading in native C++:
		////lock (O1)
		{
			...
			if (Index[5] != -1)
			{
				...
				KingOnTable[Index[5]]->KingThinking->Deep[KingOnTable[Index[5]]->KingThinking->Deep.size() - 1]->SetRowColumn(0);
	...
	void AllDraw::FullGameThinkingTreeInitialization(int ik, int j, int Order, int kind)
	{
		...
		if (kind == 1)
		{
			...
			if (SolderesOnTable[ik]->SoldierThinking->TableListSolder.size() == SolderesOnTable[ik]->SoldierThinking->Deep.size())
			{
				...
				SolderesOnTable[ik]->SoldierThinking->Deep[j]->SetRowColumn(0);
		...
		else if (kind == 2) //elephant
		{
			...
			if (ElephantOnTable[ik]->ElefantThinking->TableListElefant.size() == ElephantOnTable[ik]->ElefantThinking->Deep.size())
			{
				...
				ElephantOnTable[ik]->ElefantThinking->Deep[j]->SetRowColumn(0);
		...
		else if (kind == 3) //hourse
		{
			...
			if (HoursesOnTable[ik]->HourseThinking->TableListHourse.size() == HoursesOnTable[ik]->HourseThinking->Deep.size())
			{
				...
				HoursesOnTable[ik]->HourseThinking->Deep[j]->SetRowColumn(0);
		...
		else if (kind == 4) //Castle
		{
			...
			if (CastlesOnTable[ik]->CastleThinking->TableListCastle.size() == CastlesOnTable[ik]->CastleThinking->Deep.size())
			{
				...
				CastlesOnTable[ik]->CastleThinking->Deep[j]->SetRowColumn(0);
		...
		else if (kind == 5) //minister
		{
			...
			if (MinisterOnTable[ik]->MinisterThinking->TableListMinister.size() == MinisterOnTable[ik]->MinisterThinking->Deep.size())
			{
				...
				MinisterOnTable[ik]->MinisterThinking->Deep[j]->SetRowColumn(0);
		...
		else if (kind == 6) //king
		{
			...
			if (KingOnTable[ik]->KingThinking->TableListKing.size() == KingOnTable[ik]->KingThinking->Deep.size())
			{
				...
				KingOnTable[ik]->KingThinking->Deep[j]->SetRowColumn(0);
