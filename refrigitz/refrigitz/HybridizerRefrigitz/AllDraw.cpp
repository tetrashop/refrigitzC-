...
//ORIGINAL LINE: int[,] Th = TableHeuristic;
	...
	bool AllDraw::CheckeHuristci(int** TableS, int Order, int i, int j, int k)
	{
		...
		bool continued = false;
		ChessRules AB =  ChessRules(CurrentAStarGredyMax, MovementsAStarGreedyHeuristicFoundT, IgnoreSelfObjectsT, UsePenaltyRegardMechnisamT, BestMovmentsT, PredictHeuristicT, OnlySelfT, AStarGreedyHeuristicT, ArrangmentsChanged, 1, CloneATable(TableS), Order, SolderesOnTable[i].SoldierThinking.Row, SolderesOnTable[i].SoldierThinking.Column);
...
//C# TO C++ CONVERTER TODO TASK: There is no built-in support for multithreading in native C++:
	...
	bool AllDraw::KingDan(int** Tab, int Order)
	{
		...
		bool IsDang = false;
		ChessRules A =  ChessRules(0, MovementsAStarGreedyHeuristicFoundT, IgnoreSelfObjectsT, UsePenaltyRegardMechnisamT, BestMovmentsT, PredictHeuristicT, OnlySelfT, AStarGreedyHeuristicT, ArrangmentsChanged, Order);
